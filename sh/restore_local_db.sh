...
if [[ $(file --mime-type -b ./backend/data/dumps/moami_dump.sql) == "text/plain" ]]; then
    cat ./backend/data/dumps/moami_dump.sql | docker exec -i moami-db-1 psql -U postgres -d moami_db
else
    docker exec -i moami-db-1 pg_restore -U postgres -d moami_db < ./backend/data/dumps/moami_dump.sql
fi
