...
class MarketplaceAttributeViewSet(viewsets.ModelViewSet):
    ...
    def optimize_options(self, request, pk=None):
        ...
        prompt = f"""Категория товаров: "{attr_set_name}"
Атрибут: "{attribute.name}" (тип: {attribute.attr_type})
Всего значений: {len(options)}

Задача: оставь только те значения, которые реально используются в мировой практике для этой категории товаров. Удали редкие, экзотические, дублирующие, слишком специфичные.

Подумай: что покупатель реально выберет при фильтрации товаров в категории "{attr_set_name}"? Оставь 30-80 самых ходовых значений.
