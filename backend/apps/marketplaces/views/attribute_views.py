...
class MarketplaceAttributeViewSet(viewsets.ModelViewSet):
    ...
    def optimize_options(self, request, pk=None):
        ...
        prompt = f"""Категория товаров на маркетплейсе: "{attr_set_name}"
Атрибут: "{attribute.name}" (тип: {attribute.attr_type})
Всего значений: {len(options)}

Шаг 1: Определи, какой тип товаров продаётся в категории "{attr_set_name}" и какие форматы значений атрибута "{attribute.name}" являются СТАНДАРТНЫМИ в мировой практике для этого типа товаров.

Шаг 2: Из списка ниже оставь ТОЛЬКО значения которые соответствуют стандартным форматам.
- Оставь 30-80 значений
- Удали дубликаты с уточнениями (например "XXL (170 см)" — это дубль "XXL")
- Удали варианты с суффиксами роста/длины если есть базовый вариант без них

Формат данных: code|название
{options_text}

ОТВЕТЬ ТОЛЬКО кодами (первая колонка до |) которые ОСТАВИТЬ.
По одному коду на строку. Без пояснений, без маркеров."""
